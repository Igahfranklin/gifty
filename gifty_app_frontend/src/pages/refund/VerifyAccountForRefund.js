...
export default function VerifyAccountForRefund() {
  ...
  useEffect(() => {
    if (slug) {
      (async () => {
        try {
          await axios
            .post(apiUrls.refundGift(slug), config)
            ...
            onSubmit={async (
              ...
              try {
                const response = await axios
                  .post(apiUrls.verifyOtp(slug), JSON.stringify(values), config)
                  .then((res) => {
